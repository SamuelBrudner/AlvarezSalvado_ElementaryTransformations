#!/bin/bash
# fix_colorbar_directly.sh - Fix the ColorBar issue more thoroughly

echo "Checking current colorbar line..."
grep -n "colorbar" Code/validate_plume_setup.m | head -5

echo ""
echo "Fixing ALL colorbar references..."

# Fix all possible colorbar syntax variations
sed -i.backup2 \
    -e "s/colorbar('Label', 'Odor Concentration')/colorbar(); ylabel(cbar, 'Odor Concentration')/g" \
    -e "s/cbar = colorbar('Label', 'Odor Concentration');/cbar = colorbar(); ylabel(cbar, 'Odor Concentration');/g" \
    -e "s/colorbar('Label',/colorbar(); ylabel(cbar,/g" \
    Code/validate_plume_setup.m

echo ""
echo "Checking if fix was applied..."
grep -n "colorbar" Code/validate_plume_setup.m | head -5

# If that didn't work, let's create a completely fixed version
cat > Code/validate_plume_setup_fixed.m << 'EOF'
...
cp Code/validate_plume_setup_fixed.m Code/validate_plume_setup.m
echo ""
echo "✓ Created and installed fixed version"

# Test it
cat > test_fixed_validation.m << 'EOF'
fprintf('\n=== Testing Fixed Validation ===\n');
addpath(genpath('Code'));

try
    validate_plume_setup('test_validation_fixed.png');
    fprintf('✓ SUCCESS! Figure saved to test_validation_fixed.png\n');
catch ME
    fprintf('✗ Still failing: %s\n', ME.message);
end
exit;
EOF
...
echo "  matlab -nodisplay -r \"run('test_fixed_validation.m')\""