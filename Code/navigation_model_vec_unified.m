...
%         x(1,:) = iniX(1:ntrials); y(1,:) = iniY(1:ntrials);
    ...
    case {'openlooppulse','openlooppulsewb','openloopslope'} % Single odor pulses at 50 Hz
        x(1,:) = (rand(1,ntrials).*15)-7.5; y(1,:) = rand(1,ntrials)*4.5-29.5; triallength = 3500;
    case {'gaussian', 'Gaussian'} % Gaussian odor gradient without any wind
        x(1,:) = 20*rand(1,ntrials)-10;
        y(1,:) = 20*rand(1,ntrials)-10;
    case {'video'}
        x(1,:) = (rand(1,ntrials).*15)-7.5; y(1,:) = rand(1,ntrials)*4.5-29.5;
end
...
for i = 1:triallength
    ...
    switch environment
        ...
        case {'video'}
            ...
            out_of_plume = union(union(find(xind<1),find(xind>plume.dims(1))),union(find(yind<1),find(yind>plume.dims(2))));
            within = setdiff(1:ntrials,out_of_plume);
            odor(i,out_of_plume) = 0;
            for it = within
