...
% ``configs/project_paths.yaml`` generated by ``paths.sh``. If that file is
% missing, ``load_paths_config`` falls back to ``configs/paths.yaml`` for
...
% Ensure a paths configuration exists before continuing
projectPaths = fullfile(projectRoot, 'configs', 'project_paths.yaml');
legacyPaths = fullfile(projectRoot, 'configs', 'paths.yaml');
if ~exist(projectPaths, 'file') && ~exist(legacyPaths, 'file')
