...
echo "Note: Both plumes are in (frames, height, width) format"
...
def check_plume(filepath, dataset_name, plume_name):
    ...
    try:
        with h5py.File(filepath, 'r') as f:
            ...
            # For (frames, height, width) format
            if len(shape) == 3 and shape[0] > 1000:  # First dimension is frames
                middle_idx = shape[0] // 2
                middle_frame = data[middle_idx, :, :]  # Read frame at middle time
                print(f"  Format: (frames, height, width)")
                ...
                # Add reference arrow (0,-25) to (2,-23) in cm
                arrow_start_x = 0   # cm
                arrow_start_y = -25 # cm
                arrow_dx = 2        # cm
                arrow_dy = 2        # cm
                
                # Convert to pixel coordinates based on plume type
                if plume_name == 'Crimaldi':
                    pxscale = 0.74  # mm/pixel
                    arrow_x_px = arrow_start_x * 10 / pxscale + 108
                    arrow_y_px = -arrow_start_y * 10 / pxscale - 1  # Note: negative because image y-axis is flipped
                    arrow_dx_px = arrow_dx * 10 / pxscale
                    arrow_dy_px = -arrow_dy * 10 / pxscale
                else:  # Smoke
                    px_per_mm = 6.536
                    arrow_x_px = arrow_start_x * 10 * px_per_mm + middle_frame.shape[1]/2
                    arrow_y_px = -arrow_start_y * 10 * px_per_mm - 1
                    arrow_dx_px = arrow_dx * 10 * px_per_mm
                    arrow_dy_px = -arrow_dy * 10 * px_per_mm
                
                # Add arrows to both plots
                for ax in [ax1, ax2]:
                    ax.arrow(arrow_x_px, arrow_y_px, arrow_dx_px, arrow_dy_px,
                            head_width=20, head_length=15, fc='red', ec='red', 
                            linewidth=2, alpha=0.8)
                    ax.text(arrow_x_px-30, arrow_y_px+30, '(0,-25) cm', 
                           color='red', fontsize=10, weight='bold',
                           bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.7))
                    ax.plot(arrow_x_px, arrow_y_px, 'ro', markersize=8)  # Mark start point
