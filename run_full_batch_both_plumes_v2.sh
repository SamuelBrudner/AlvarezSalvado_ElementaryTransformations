#!/bin/bash
#SBATCH --job-name=full_both_v2
#SBATCH --output=logs/full_batch_v2_%A_%a.out
#SBATCH --error=logs/full_batch_v2_%A_%a.err
#SBATCH --array=0-39%10  # 40 jobs: 2 plumes x 20 jobs per plume, max 10 concurrent (reduced from 20)
#SBATCH --time=6:00:00
#SBATCH --mem=80G  # Increased from 64G
#SBATCH --partition=day

set -euo pipefail

# Create directories
mkdir -p logs data/raw/full_batch_v2 data/processed/full_batch_v2
...
MATLAB_SCRIPT=$(mktemp full_batch_v2_XXXX.m)
...
output_base = 'data/raw/full_batch_v2/${PLUME_NAME}';
...
for agent = $START_AGENT:$END_AGENT
    ...
    try
        ...
        % Export to CSV/JSON
        export_dir = fullfile('data/processed/full_batch_v2', '${PLUME_NAME}', sprintf('agent_%04d', agent));
...
echo "Full batch job V2 completed"
