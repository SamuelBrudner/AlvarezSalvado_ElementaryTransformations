% fix_smoke_dimensions.m - Fix dimension ordering in smoke HDF5 file
% With immediate output (no buffering)

fprintf('\n=== Fixing Smoke Plume Dimension Order ===\n\n');
drawnow('update');  % Force immediate output
...
fprintf('The fix will:\n');
fprintf('  - Read the data (this may take time for 25GB file)\n');
fprintf('  - Permute dimensions from [frames,width,height] to [width,height,frames]\n');
fprintf('  - Save to a new file: smoke_1a_rotated_3d_fixed.h5\n\n');
drawnow('update');

reply = input('Proceed with fix? (yes/no): ', 's');
