...
%   Loads the ``project_paths.yaml`` configuration (generated by ``paths.sh``).
%   If that file does not exist, the legacy ``paths.yaml`` is loaded for
%   backward compatibility. Expands any environment variables and returns a
%   struct with the configuration values.
    ...
    % Fallback to legacy paths.yaml if project_paths.yaml is missing
    if ~exist(configFile, 'file')
        legacyFile = fullfile(projectRoot, 'configs', 'paths.yaml');
        if exist(legacyFile, 'file')
            warning('load_paths_config:LegacyFile', ...
                'Using legacy paths.yaml. Please migrate to project_paths.yaml');
